...
void image::render(HDC hdc, int destX, int destY, int sourX, int sourY, int sourWidth, int sourHeight,int scale)
{
	if (_trans)
	{
		GdiTransparentBlt(hdc,		//복사될 DC영역
...
void image::scaleFrameRender(HDC hdc, int destX, int destY,int scale)
{
	if (_trans)
	{
		GdiTransparentBlt(
...
void image::alphaScaleRender(HDC hdc, int destX, int destY, float scale, BYTE alpha)
{
	//실제 이미지에 알파블렌드를 접목시켜주는 함수
	_blendFunc.SourceConstantAlpha = alpha;		//0 ~ 255 로 투명도를 가진다
	...
	if (_trans)
	{
		...
		GdiTransparentBlt(_blendImage->hMemDC, 0, 0, _imageInfo->width  * scale, _imageInfo->height * scale, _imageInfo->hMemDC, 0, 0, _imageInfo->width, _imageInfo->height, _transColor);

		AlphaBlend(hdc, destX, destY, _imageInfo->width * scale, _imageInfo->height * scale, _blendImage->hMemDC, 0, 0, _imageInfo->width, _imageInfo->height, _blendFunc);
	}
	else
	{
		AlphaBlend(hdc, destX, destY, _imageInfo->width, _imageInfo->height, _imageInfo->hMemDC, 0, 0, _imageInfo->width, _imageInfo->height, _blendFunc);
...
void image::alphaScaleFrameRender(HDC hdc, int destX, int destY,float scale, BYTE alpha)
{
	//실제 이미지에 알파블렌드를 접목시켜주는 함수
	_blendFunc.SourceConstantAlpha = alpha;		//0 ~ 255 로 투명도를 가진다
	...
	//	AlphaBlend(hdc, destX, destY, _imageInfo->frameWidth * scale, _imageInfo->frameHeight * scale, _blendImage->hMemDC, 0, 0, _imageInfo->currentFrameX * _imageInfo->frameWidth, _imageInfo->currentFrameY * _imageInfo->frameHeight, _blendFunc);
	}
	else
	{
		AlphaBlend(hdc, destX, destY, _imageInfo->width, _imageInfo->height, _imageInfo->hMemDC, 0, 0, _imageInfo->width, _imageInfo->height, _blendFunc);
...
void image::aniRender(HDC hdc, int destX, int destY, animation * ani,float scale)
{
	render(hdc, destX, destY, ani->getFramePos().x, ani->getFramePos().y, ani->getFrameWidth(), ani->getFrameHeight(),scale);
}
